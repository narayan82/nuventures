<?php
/**
 * Portfolio page NuJourney carousel.
 *
 * @package NuVentures
 */
...
<section class="portfolio-journeys" data-portfolio-journeys aria-labelledby="portfolio-journeys-title">
    <h1 class="portfolio-page__heading" id="portfolio-journeys-title"><?php esc_html_e('NuJourney with the founders', 'nuventures'); ?></h1>

    <div class="portfolio-journeys__track" data-portfolio-journeys-track tabindex="0" aria-label="<?php esc_attr_e('NuJourney', 'nuventures'); ?>">
        <?php
        while ($journeys_query->have_posts()) :
            ...
            $logo        = $resolve_image(get_field('logo', $journey_id), get_the_title() . ' logo');
            ?>
            <a class="portfolio-journeys__card" href="<?php echo esc_url(get_permalink($journey_id)); ?>">
                ...
                <span class="portfolio-journeys__content">
                    <span class="portfolio-journeys__heading">
                        <span class="portfolio-journeys__title"><?php echo esc_html($title ? $title : get_the_title()); ?></span>

                        <?php if ($logo['id'] || $logo['url']) : ?>
                            <span class="portfolio-journeys__logo">
                                <?php if ($logo['id']) : ?>
                                    <?php
                                    echo wp_get_attachment_image(
                                        $logo['id'],
                                        ...
                                        false,
                                        array(
                                            'class'    => 'portfolio-journeys__logo-image',
                                            'alt'      => $logo['alt'],
                                            'loading'  => 'lazy',
                                            'decoding' => 'async',
                                            'sizes'    => '120px',
                                    ...
                                    ?>
                                <?php else : ?>
                                    <img class="portfolio-journeys__logo-image" src="<?php echo esc_url($logo['url']); ?>" alt="<?php echo esc_attr($logo['alt']); ?>" loading="lazy" decoding="async">
                                <?php endif; ?>
                            </span>
                        <?php endif; ?>
                    </span>
